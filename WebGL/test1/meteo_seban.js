...
const float PREC = 255.0/254.0;     // fixme:这个要跟后面的一致
...
uniform float u_dataOffset; // 偏移值
uniform float u_isoline;     // 等值线数值间距
uniform int u_gradient;     // 是否渐变
...
//            float val = length(mix(u_min,u_max,texture2D(u_data, c).xy*PREC));  //  通过图片的纹理坐标c获得各通道像素值,然后线性混合,在通过length方法求矢量和:如求u,v矢量和.得到色卡横坐标,即gfs数据值的大小
            ...
            vec2 eachValue;
            for(int i=0;i<9;i++){
                eachValue += mix(u_min,u_max,texture2D(u_data, c).xy*PREC)*u_kernel[i];
                kernelWeight +=u_kernel[i];
            }
            eachValue /=kernelWeight;
            ...
            // fixme:结合版本
            float colorPos;
            if((bool(u_gradient))){         // 渐变
                colorPos = between(u_cmm[0],u_cmm[1],val);    // 通过色卡横坐标val得到色卡纹理坐标
            }else{                                      // 不渐变：这个不是第一版不渐变色卡，这个是专门为配合与热力图同时显示所生成的色卡，所以要特殊处理一下
                colorPos = between(u_cmm[0],u_cmm[1],floor((val+u_dataOffset)/u_isoline)*u_isoline-u_dataOffset);    // 通过色卡横坐标val得到色卡纹理坐标
            }
...
//             val = texture2D(u_data, point[4]).xy*PREC;
...
uniform sampler2D u_frameShade;     // 热力图纹理
uniform vec2 u_cmm;         // 色卡横坐标最小最大,横坐标等同于gfs等数据的大小
varying vec2 v_pos;         // todo：传进来的WebGL坐标系的点，要在main()进行转换？？
float unpackDepth(const in vec4 rgbaDepth) {
    const vec4 bitShift = vec4(1.0, 1.0/256.0, 1.0/(256.0*256.0), 1.0/(256.0*256.0*256.0));
...
void main(){ 
    ...
    gl_FragColor=texture2D(u_frameShade, texCoord);
...
class Meteo {
    constructor(map) {
        ...
        this.fontInfo = {
            ...
            isolineValue:0.5,                // 等值线间距
            dataOffset:0,                   // 色卡偏移值
            isGradient:false,               // 色卡是否渐变
        };
    ...
    _init() {
        ...
        map.on('resize', (e) => {
            ...
            this.fboS = this.initFramebufferObject(this.gl);
    ...
    _initGL() {
        const gl = this.gl;
        const vertShaderS = createShader(gl, gl.VERTEX_SHADER, vertS);
        const fragShaderS = createShader(gl, gl.FRAGMENT_SHADER, fragS);
        this.programS = createProgram(gl, vertShaderS, fragShaderS);
        ...
        const fboS=this.fboS = this.initFramebufferObject(this.gl);
        if (!fboS) {
        ...
        this.gl.uniform1f(this.programS.u_opacity, 0.8);
        this.gl.uniform1f(this.programS.u_dataOffset, this.fontInfo.dataOffset);
        this.gl.uniform1f(this.programS.u_isoline, this.fontInfo.isolineValue);
        if (this.fontInfo.isGradient) {
            this.gl.uniform1i(this.programS.u_gradient, 1);
        } else {
            this.gl.uniform1i(this.programS.u_gradient, 0);
        }
        // fixme:1
        this.pixels = new Uint8Array(this.gl.drawingBufferWidth * this.gl.drawingBufferHeight * 4);
        /*// fixme:2
    ...
    setColor(color) {
        ...
        const color2D = createColorRamp(color,this.fontInfo.isolineValue, this.fontInfo.dataOffset); // 画色卡
        const colorTexture = createTexture(this.gl, this.gl.LINEAR, color2D, color2D.length / 4, 1, TEXTURE_INDEX_COLOR);
        this.gl.useProgram(this.programS);
        this.gl.uniform1i(this.programS.u_color, TEXTURE_INDEX_COLOR);
        this.gl.uniform2fv(this.programS.u_cmm, new Float32Array([color[0][0], color[color.length - 1][0]]));
    }
    ...
    load(url, vector) {
        ...
        MeteoImage.load(url).then((meteo) => {
            ...
            this.gl.useProgram(this.programS);
            this.gl.uniform1i(this.programS.u_data, TEXTURE_INDEX_DATA);
            this.gl.uniform3fv(this.programS.u_lon, meteo.lon);
            this.gl.uniform3fv(this.programS.u_lat, meteo.lat);
            this.gl.uniform2fv(this.programS.u_min, [meteo.minAndMax[0][0], vector ? meteo.minAndMax[1][0] : 0]);
            this.gl.uniform2fv(this.programS.u_max, [meteo.minAndMax[0][1], vector ? meteo.minAndMax[1][1] : 0]);
            this.gl.uniform1fv(this.programS["u_kernel[0]"], new Float32Array([1.0/16.0,2.0/16.0,1.0/16.0,
                2.0/16.0,2.0/16.0,2.0/16.0,
                1.0/16.0,2.0/16.0,1.0/16.0]));
    ...
    _frame() {
        ...
        const gl = this.gl;
        // fixme:(0)单纯的帧缓冲区热力图
        this.gl.activeTexture(this.gl.TEXTURE0+TEXTURE_FRAME_SHADE); // Set a texture object to the texture unit
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.fboS.texture); // fixme:这里放的是帧缓冲区的纹理图像
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.fboS);
        gl.viewport(0, 0, this.gl.canvas.width, this.gl.canvas.height);
        this.gl.clearColor(0, 0, 0, 0);
        this.gl.clear(this.gl.COLOR_BUFFER_BIT);
        gl.useProgram(this.programS);
        gl.uniformMatrix4fv(this.programS.u_matrix_invert, false, this._matrixInvert());
        this.gl.uniform2fv(this.programS.u_textureSize, [this.gl.canvas.width, this.gl.canvas.height]);
        bindAttribute(gl, this.posBuffer, this.programS.a_position, 2);
            ...
            for(let x=0;x<perFontWidth;x++){
                ...
                for(let y=0;y<paramY.byteLength;y++){
                    ...
                    if(this.pixels[pixelsIndex]!==0||this.pixels[pixelsIndex+1]!==0||this.pixels[pixelsIndex+2]!==0||this.pixels[pixelsIndex+3]!==0){
                        ...
                        break;
                    }else if(x===perFontWidth-1&&y===paramY.byteLength-1){
                        fontArray.push(-1);
                        break;
                    }
                }
        ...
        this.gl.activeTexture(this.gl.TEXTURE0+TEXTURE_FRAME_SHADE); // Set a texture object to the texture unit
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.fboS.texture); // fixme:这里放的是帧缓冲区的纹理图像
        this.gl.uniform1i(this.programAll.u_frameShade, TEXTURE_FRAME_SHADE);
        gl.drawArrays(this.gl.TRIANGLES, 0, 6);
    }
