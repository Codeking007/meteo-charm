...
uniform sampler2D u_border;         // 边界的纹理单元
...
void main(){ 
    ...
    if(texture2D(u_border, texCoord).x!=0.0){       // 如果r通道是1，就代表要画这个点
...
class Meteo{
    constructor(map) {
        ...
        this.fontInfo = {
            ...
            isolineValue:2,                  // 等值线间距
            dataFormat:(data)=>{return Math.round(data);},      // 处理显示的值
            dataOffset:(data)=>{return (data-273);}             // 值的偏移值
            /*isolineValue:200,                  // 等值线间距
            dataFormat:(data)=>{return Math.round(data/100);},      // 处理显示的值
            dataOffset:(data)=>{return (data);}             // 值的偏移值*/
    ...
    load(url, vector){
        ...
        MeteoImage.load(url).then((meteo) => {
            this.gl.useProgram(this.program);
            for(let i=0;i<3;i++){        // fixme：给值作偏移，如：温度-273=摄氏度
                for(let j=0;j<2;j++){
                    if((i===0)||(i===1&&vector)){     // fixme:判断第二个通道是否存在最大最小值，不存在就让他是0，不偏移
                        meteo.minAndMax[i][j]=this.fontInfo.dataOffset(meteo.minAndMax[i][j]);
                    }
                }
            }
            this.meteo = meteo;
    ...
    showHLPointsWithMapbox(){
        ...
        /*la.forEach((value, key, map) =>  {
            ps.push(turf.point([value[1], value[2]], {v:-1,val:Math.round(min+(max-min)*value[0]/254),
                description:"L\r\n" +this.fontInfo.dataFormat(min+(max-min)*value[0]/254).toFixed(0)    // fixme:单位百帕
            }));
        });
        ha.forEach((value, key, map) => {
            ps.push(turf.point([value[1], value[2]], {v:1,val:Math.round(min+(max-min)*(value[0] as any)/254),
                description:"H\n\r" +this.fontInfo.dataFormat(min+(max-min)*value[0]/254).toFixed(0)    // fixme:单位百帕
            }));
        });*/
...
*/
    ...
    drawBorder(){
        debugger
        let martix=this._matrix();
        function posx(lon){
            return (lon+180.0)/360.0;
        }
        function posy(lat){
            return 0.5-Math.log((1.0+Math.sin(lat*Math.PI/180.0))/(1.0-Math.sin(lat*Math.PI/180.0)))/(4.0*Math.PI);
        }
        let testCoord=[[[157,20],[157,40],[125,40],[125,59],[75,59],[75,40],[52,40],[52,20],[75,20],[75,7],[125,7],[125,20],[157,20]]];
        let testCoord1=[[[125,40],[75,40],[75,20],[125,20],[125,40]]];
        // fixme:画边界
        const canvasBorder = document.createElement('canvas');
        ...
        for(let j=0;j<testCoord.length;j++){
            contextBorder.beginPath();
            for(let i=0;i<testCoord[j].length;i++){
                let currentCoord=testCoord[j][i];
                let texture=new Float32Array([posx(currentCoord[0]),posy(currentCoord[1]),0,1]);
                let p0 = mat4.identity(new Float32Array(4)); // 定义为单元阵
                mat4.multiply(p0, martix, texture);
                if(i===0){
                    contextBorder.moveTo((p0[0]+1.0)/2.0*this.gl.canvas.width, (p0[1]+1.0)/2.0*this.gl.canvas.height);
                }else{
                    contextBorder.lineTo((p0[0]+1.0)/2.0*this.gl.canvas.width, (p0[1]+1.0)/2.0*this.gl.canvas.height);
                }
            }
            //不关闭路径路径会一直保留下去
        ...
        for(let j=0;j<testCoord1.length;j++){
            contextBorder.beginPath();
            for(let i=0;i<testCoord1[j].length;i++){
                let currentCoord=testCoord1[j][i];
                let texture=new Float32Array([posx(currentCoord[0]),posy(currentCoord[1]),0,1]);
                let p0 = mat4.identity(new Float32Array(4)); // 定义为单元阵
                mat4.multiply(p0, martix, texture);
                if(i===0){
        ...
        let BorderData=new Uint8Array(contextBorder.getImageData(0, 0, this.gl.canvas.width, this.gl.canvas.height).data);
        this.borderTexture = createTexture(this.gl, this.gl.NEAREST, BorderData, this.gl.canvas.width, this.gl.canvas.height, TEXTURE_INDEX_BORDER, this.gl.RGBA);
    }
    ...
    _frame(){
        ...
        this.gl.activeTexture(this.gl.TEXTURE0+TEXTURE_INDEX_BORDER);
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.borderTexture);
        this.gl.uniform1i(this.programAll.u_border, TEXTURE_INDEX_BORDER);
