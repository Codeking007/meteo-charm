...
const float u_precision=5.0;    // 保留几位小数
const int numLength=int(log2(abs(length(-85234.62457)))/log2(10.0)+1.0);  // 有几个整数位// log10(x)+1
const int preci=numLength+int(u_precision);   // 总共几位有效数字
const float a=1.0/u_num.x;
const float b=1.0/u_num.y;
const float c1=a*0.3;
const float c2=b*0.1;
const float fontNum=u_precision>0.0?float(preci+2):float(preci+1);
const float d=c1/fontNum;
vec4 getFontColor(vec2 texCoord,float testValue,float wei){
    if (testValue >= wei) {
...
//            float val = length(mix(u_min,u_max,texture2D(u_data, c).xy*PREC));  //  通过图片的纹理坐标c获得各通道像素值,然后线性混合,在通过length方法求矢量和:如求u,v矢量和.得到色卡横坐标,即gfs数据值的大小
            ...
            float testValue = -97531.24681357;
            vec4 fontColor;
            if(mod(texCoord.x,a)<=c1&&mod(texCoord.y,b)<=c2){
                for(int i=0;i<int(fontNum);i++){
                    if(mod(texCoord.x,a)/c1<d/c1*float(i+1)){
                        if(i+1==1){   // 符号位
                            if(testValue<0.0){   // 负数
                                float xOffset = mod(mod(texCoord.x,a),d)/d*(1.0/60.0);      // 这样写也行==>float xOffset = mod(mod(texCoord.x,a)/d*(1.0/16.0),1.0/16.0); 
                                float yOffset = mod(texCoord.y,b)/c2; 
                            ...
                            fontColor =getFontColor(texCoord,abs(testValue),pow(10.0,float(numLength-i)));
                        }else if(i+1==numLength+2){ // 小数点
                            float xOffset = mod(mod(texCoord.x,a),d)/d*(1.0/17.0);      // 这样写也行==>float xOffset = mod(mod(texCoord.x,a)/d*(1.0/16.0),1.0/16.0); 
                            float yOffset = mod(texCoord.y,b)/c2; 
                            texCoord.x = 10.0 / 16.0 + xOffset;
                            texCoord.y = 1.0-(0.0 + yOffset);
                            fontColor = vec4(1.0,1.0,1.0,2.0)-texture2D(u_numbers, texCoord);
                        }else if(i+1>=numLength+3){ // 小数位
//                            fontColor =getFontColor(texCoord,abs(testValue),pow(10.0,float(numLength+1-i)));
                            float xOffset = mod(mod(texCoord.x,a),d)/d*(1.0/16.0);      // 这样写也行==>float xOffset = mod(mod(texCoord.x,a)/d*(1.0/16.0),1.0/16.0); 
                            float yOffset = mod(texCoord.y,b)/c2; 
                            texCoord.x = getNumber(abs(testValue), pow(10.0,float(numLength+1-i))) / 16.0 + xOffset;
                            texCoord.y = 1.0-(0.0 + yOffset);
                            fontColor = vec4(1.0,1.0,1.0,2.0)-texture2D(u_numbers, texCoord);
                        }
                        break;
