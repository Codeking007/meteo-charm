...
void main(){  
        ...
        vec2 point[9];
        point[0]=v_index_texCoord + vec2(-onePixel.x, onePixel.y);
        point[1]=v_index_texCoord + vec2(0.0, onePixel.y);
        point[2]=v_index_texCoord + vec2(onePixel.x, onePixel.y);
        point[3]=v_index_texCoord + vec2(-onePixel.x, 0.0);
        point[4]=v_index_texCoord + vec2(0.0, 0.0);
        point[5]=v_index_texCoord + vec2(onePixel.x, 0.0);
        point[6]=v_index_texCoord + vec2(-onePixel.x, -onePixel.y);
        point[7]=v_index_texCoord + vec2(0.0, -onePixel.y);
        point[8]=v_index_texCoord + vec2(onePixel.x, -onePixel.y);
        float kernelWeight;
        vec2 val;
        for(int i=0;i<9;i++){
...
class Meteo{
    ...
    showHLPointsWithMapbox(){
        ...
        for(var index=0;index<this.lonlatData.length;index+=4){
            ...
            let currentLonLat= [currentLonLatRate[0]*(this.u_lon_imageLonLat[1]-this.u_lon_imageLonLat[0])+this.u_lon_imageLonLat[0],currentLonLatRate[1]*(this.u_lat_imageLonLat[1]-this.u_lat_imageLonLat[0])+this.u_lat_imageLonLat[0]];
