...
void main(){
    ...
    if(valid(c)){  
        ...
        val = texture2D(u_data, c).xy*PREC;
...
class Meteo {
    constructor(map) {
        this.mapLayerId = "meteo0701";
        this.mapSourceId = "meteo0701";
        this.map = map;
        this.fontInfo = {
            letterWidth: 32,
            letterHeight: 32,
        ...
        this._init();
        this._initGL();
        this._render(this.mapLayerId, this.mapSourceId);    // 加载重写的mapbox源码==>生成瓦片的
    }

    done(err, img, tile, callback) {
        delete tile.request;
        if (tile.aborted) {
            tile.state = 'unloaded';
            callback(null);
    ...
    _render(mapLayerId, mapSourceId) {
