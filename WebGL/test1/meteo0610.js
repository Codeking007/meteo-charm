...
class Meteo {
    constructor(map) {
        ...
        // 找到画布
        let textCanvas = this.textCanvas = document.getElementById("text");
        // 创建一个二维上下文
        let textCtx = this.textCtx = textCanvas.getContext("2d");
        this.map = map;
        this.imageNum = 2;
        this.fontInfo = {
            letterWidth: 32,
...
*/
            ...
            for (let widthNumber = 0; widthNumber < this.fontInfo.fontWidthNumber; widthNumber++) {
                for (let y = startHeight; y < endHeight; y++) {
                    ...
                    for (let x = startWidth; x < endWidth; x++) {
                        ...
                        if (this.pixels[pixelsIndex] !== 0 || this.pixels[pixelsIndex + 1] !== 0 || this.pixels[pixelsIndex + 2] !== 0 || this.pixels[pixelsIndex + 3] !== 0) {
                            let rate = (this.pixels[pixelsIndex] * 1.0 + this.pixels[pixelsIndex + 1] / 256.0 + this.pixels[pixelsIndex + 2] / (256.0 * 256.0) + this.pixels[pixelsIndex + 3] / (256.0 * 256.0 * 256.0)) / 255.0;
                            fontArray.push([pixelsIndex / 4, Math.round((rate * (this.cmm[1] - this.cmm[0]) + this.cmm[0]) / this.fontInfo.isolineValue) * this.fontInfo.isolineValue, indexX, indexY]);
                            break;
                        } else if (x === endWidth - 1 && y === endHeight - 1) {  // fixme:for循环的最后一个值，不是范围的最后一个值
                            fontArray.push(-1);
                            break;
                        }
                    }
    ...
    showVerticesWithMapbox(fontInfo, fontArrays) {
        ...
        this.textCtx.fillStyle = "#00ffff";
        this.textCtx.textAlign = "center";
        this.textCtx.font="10px Arial";
        this.geojson1 = {
        ...
        for (let i = 0; i < fontArrays.length; i++) {
            ...
            if (fontArray.length === 4) {
                this.textCtx.fillText(fontArray[1], fontArray[2], this.textCanvas.height - fontArray[3]);
            }
        }
        this.textCtx.font="15px Arial";
    ...
    computePixelCoordinatesAndDrawText(pointArray,value,highOrLowPointSign){
        // 计算经度-180~180的像素坐标
    ...
    signHighOrLowPointText(pos,value,highOrLowPointSign){
        if(pos.x>=0&&pos.x<=this.gl.canvas.width/this.devicePixelRatio&&pos.y>=0&&pos.y<=this.gl.canvas.height/this.devicePixelRatio){
            ...
            this.textCtx.fillText(value, pos.x*this.devicePixelRatio, pos.y*this.devicePixelRatio+15);
        }
    }
