...
// fixme:实现雾化的方式有很多种，这里使用最简单的一种：线性雾化
// fixme:<雾化因子>=(<终点与视点的距离>-<当前点与视点的距离>)/(<终点与视点的距离>-<起点与视点的距离>)
// fixme:<片元颜色>=<物体表面颜色>*<雾化因子>+<雾的颜色>*(1-<雾化因子>)
// fixme:(1)顶点着色器计算出当前顶点与视点的距离，并传入片元着色器
// fixme:(2)片元着色器根据片元与视点的距离，计算雾化因子，最终计算出片元的颜色
// fixme:程序向着色器传入了视点在世界坐标系下的坐标
...
var VSHADER_SOURCE =
  ...
  uniform vec4 u_Eye;   // fixme:世界坐标  // Position of eye point (world coordinates)
...
var FSHADER_SOURCE =
  ...
  void main() {
     ...
     // fixme:计算雾化因子
     // fixme:clamp()函数：将第一个参数的值限制在第二个和第三个参数的构成区间内。如果值在区间内，函数就直接返回这个值，超出范围的显示区间最大最小值
