...
export class Isoline implements IWebGL{
    ...
    loadMeteo(meteo: any, isolineParams: any, precision: any) {
        this.isolineRealtedParams = isolineParams.isoline;
        const contourDelta = this.contourDelta = isolineParams.isoline.delta(null);
        const contourScale = this.contourScale = precision / 100;
        const w = meteo.width;
        ...
        if (this.showFontWithMapbox) {
            ...
            for (let v of la.values())
                ps.push(turf.point([v[1], v[2]], {
                    ...
                    description: "L\r\n" + isolineParams.isoline.dataFormat(min + (max - min) * v[0] / 254)
                }));
            for (let v of ha.values())
                ps.push(turf.point([v[1], v[2]], {
                    v: 1, val: Math.round(min + (max - min) * v[0] / 254),
                    description: "H\n\r" + isolineParams.isoline.dataFormat(min + (max - min) * v[0] / 254)
                }));
        ...
        this.meteo = meteo;
        // 形成数据纹理
        this.dataTexture = this.wgl.createTexture(TEXTURE_INDEX_DATA, this.meteo.width, this.meteo.height, this.gl.RGBA, this.gl.RGBA, this.gl.UNSIGNED_BYTE, this.gl.LINEAR, this.meteo.data);
        ...
        this.program1.use();
        this.wgl.gl.uniform2fv(this.program1.uniform.u_min, [meteo.minAndMax[0][0], isolineParams.computeAsVector[1] ? meteo.minAndMax[1][0] : 0]);
        this.wgl.gl.uniform2fv(this.program1.uniform.u_max, [meteo.minAndMax[0][1], isolineParams.computeAsVector[1] ? meteo.minAndMax[1][1] : 0]);
        if (isolineParams.computeAsVector[1]) {      // fixme:如果是单通道，就得判断是正是负，如果是双通道，意味着是矢量，就不用管正负了
            this.cmm = [Math.pow((Math.pow(meteo.minAndMax[0][0], 2) + Math.pow(isolineParams.computeAsVector[1] ? meteo.minAndMax[1][0] : 0, 2)), 0.5), Math.pow((Math.pow(meteo.minAndMax[0][1], 2) + Math.pow(isolineParams.computeAsVector[1] ? meteo.minAndMax[1][1] : 0, 2)), 0.5)];
        } else {
    ...
    showVerticesWithMapbox(fontInfo:any, fontArrays:any) {
        ...
        if (!this.showFontWithMapbox) {
            ...
            for (let v of this.la.values()) {
                let value = (Math.round(this.meteo.minAndMax[0][0] + (this.meteo.minAndMax[0][1] - this.meteo.minAndMax[0][0]) * v[0] / 254) / 100).toFixed(0);
