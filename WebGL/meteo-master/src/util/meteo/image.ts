...
import {
    ...
    MeteoTypeIndex, MeteoSourceIndex
} from "@/util/meteo/thor-meteo/meteo";
import {MeteoResultInterface} from "@/util/meteo/thor-meteo";
...
export class MeteoImage {
    ...
    getCoord(uintArray: Uint8ClampedArray) {
        ...
        for (let m = 0; m < d.length; m++)      // 第m个值（最小值，最大值，精度中的一个）
            for (let n = 0; n < 4; n++)
                u[4 * m + n] = uintArray[4 * n + m];    // m为第几行。 n为第几列。 4为一列有四个，即rgba。
        return d;
    }

    getMinAndMax(uintArray: Uint8ClampedArray) {
        let mm = new Array(3);
        for (let n = 0; n < mm.length; n++) {   // 第n个气象类型
...
export class MeteoArrayBuffer {
    load(period: string, meteoTypeConfiguration: MeteoTypeConfigurationInterface, meteoSourcePrecision: number,): Promise<Array<Float32Array>> {
        debugger
        ...
        for (let sourceIndex = 0; sourceIndex < meteoTypeConfiguration.meteoSourceConfiguration.length; sourceIndex++) {
            ...
            for (let typeIndex = 0; typeIndex < meteoTypeConfiguration.meteoTypeIndex.length; typeIndex++) {
                ...
                let meteoTypeUrl = MeteoSourceConfiguration.URL_PREFIX + "/"
                    + currentMeteoSourceConfiguration.meteoSourceIndex + "/"
                    + currentMeteoTypeIndex + "/"
                    + (meteoSourcePrecision == 1 ? 0 : meteoSourcePrecision).toString().replace(/\./g, "_") + "/"
                    + period;
    ...
    resolveData(originalData: Float32Array, meteoSourceConfiguration: MeteoSourceConfigurationInterface, meteoSourcePrecision: number,) {
        ...
        let re: MeteoResultInterface = {
            ...
            height: (meteoSourceConfiguration.latTo - meteoSourceConfiguration.latFrom) / meteoSourcePrecision + 1,
            data: pixelData,
            minAndMax: minAndMax,
            lon: new Float32Array([meteoSourceConfiguration.lonFrom, meteoSourceConfiguration.lonTo, meteoSourcePrecision]),
            lat: new Float32Array([meteoSourceConfiguration.latFrom, meteoSourceConfiguration.latTo, meteoSourcePrecision]),
        };
        return re;
    }
