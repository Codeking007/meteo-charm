...
export class Isoline implements IWebGL {
    ...
    constructor(map: mapboxgl.Map, isolineTextId: string) {
        ...
        // 找到画布
        this.textCanvas = document.getElementById(isolineTextId);
        // 创建一个二维上下文
    ...
    _init() {
        ...
        const params = {depth: false, stencil: false, antialias: false};//, preserveDrawingBuffer: true todo 截图等值线截不上，没有用
    ...
    showVerticesWithMapbox(fontInfo: any, fontArrays: any) {
        ...
        if (clickedLayer) {
            ...
            this.setZIndex(<string>this.canvas.style.zIndex);
        } else {
            this.map.addLayer({
                'id': this.valID,
                'type': 'symbol',
    ...
    setZIndex(z: string) {
        ...
        if (z == "-1") {    // 被地图覆盖
            if (this.map.getLayer(this.lhID)) {
                this.map.moveLayer(this.lhID, BoxMap.BASELAYER);
            }
            if (this.map.getLayer(this.lhID) && this.map.getLayer(this.valID)) {
                this.map.moveLayer(this.valID, this.lhID);
            }
        } else {       // 不被地图覆盖eteoOptions.DISPLAY_TYPE.shade || displayType === MeteoOptions.DISPLAY_TYPE.shade1) {
            if (this.map.getLayer(this.lhID) && this.map.getLayer(Meteo.SHADE_GRID_CUSTOMLAYER_ID)) {
                this.map.moveLayer(Meteo.SHADE_GRID_CUSTOMLAYER_ID, this.lhID);
            } else if (this.map.getLayer(this.lhID)) {
                this.map.moveLayer(BoxMap.BASELAYER, this.lhID);
            }
            if (this.map.getLayer(this.lhID) && this.map.getLayer(this.valID)) {
                this.map.moveLayer(this.lhID, this.valID);
            }
        }
    }
