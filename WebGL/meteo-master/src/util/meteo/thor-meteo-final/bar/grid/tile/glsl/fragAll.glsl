...
void main(){
    ...
    if (rateUV.r!=1.0&&rateUV.g!=1.0&&rateUV.b!=1.0){ // fixme:有效的值ba通道都设的1.0，如果不是1.0就不画了，因为如果画的话，取出来的值都是0.0，而后面有个mix方法，会取u_min的值，这样就错了，那些中心点没值也画出来东西了
        float val1 = (rateUV.x*256.0*255.0+rateUV.z*255.0)/65535.0*PREC;
        float val2 = (rateUV.y*256.0*255.0+rateUV.w*255.0)/65535.0*PREC;
        ...
        vec2 u_windBar_radix=vec2(320.0, 320.0);// 风杆图片宽度高度
        vec2 u_windBar_perRadix=vec2(64.0, 64.0);// 风杆图标宽度高度
