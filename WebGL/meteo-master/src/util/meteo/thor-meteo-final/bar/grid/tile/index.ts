...
export class TileBar implements IWebGL {
    ...
    setZIndex(z: string) {
        if (this.map.getLayer(this.mapLayerId) != null) {
            if (z == "-1") {    // 被地图覆盖
                this.map.moveLayer(this.mapLayerId, BoxMap.BASELAYER);
            } else {       // 不被地图覆盖eteoOptions.DISPLAY_TYPE.shade || displayType === MeteoOptions.DISPLAY_TYPE.shade1) {
                this.map.moveLayer(BoxMap.BASELAYER, this.mapLayerId);
            }
        }
