...
void main() {
    ...
    if (tp.y>=0.0&&tp.y<=1.0){
        ...
        if (valid(c)&&texture2D(u_data, c).a==1.0){ // fixme：当前位置还应该有气象数据才能去算下一点的位置
            ...
            target=(target.xy)/(2.0);
            point.xz = sum(point.xz, target.x/255.0);
            point.yw = sum(point.yw, target.y/255.0);

            /*target =vec2(uTo, vTo*cos(radians(u_pitch)));
            vec2 texturePoint_to=pos+vec2(target.x, -target.y)/length(u_max)*u_speed_factor/1000.0;
