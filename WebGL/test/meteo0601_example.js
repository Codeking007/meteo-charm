...
void main(){
    ...
    if(tp.y<1.0&&tp.y>0.0){
        ...
        if(valid(c)){ 
//            float k=-u_textureSize.y/u_textureSize.x;
//            float k=u_textureSize.x/u_textureSize.y;
            float k=0.0;
            float b=0.02;
...
//            float val = length(mix(u_min,u_max,texture2D(u_data, c).xy*PREC));  //  通过图片的纹理坐标c获得各通道像素值,然后线性混合,在通过length方法求矢量和:如求u,v矢量和.得到色卡横坐标,即gfs数据值的大小
            ...
            /*if(mod(c.y-k*c.x,0.1)<0.001){
                gl_FragColor = vec4(1.0,1.0,1.0,1.0);
            }*/
            vec2 tex=v_pos/2.0+0.5;
            if(mod(tex.y-k*tex.x,b)<0.002){
                gl_FragColor = vec4(1.0,1.0,1.0,1.0);
            }
            /*vec2 tex=v_pos/2.0+0.5;
            if(mod(tex.y*u_textureSize.y*u_textureSize.x+tex.x*u_textureSize.x,40.0)<=0.5){
                gl_FragColor = vec4(1.0,1.0,1.0,1.0);
            }*/
        }
    }
