...
const TEXTURE_FRAMEKERNEL = 4;
const TEXTURE_FRAMEISOLINE = 5;
const vert = `
...
varying vec2 v_index_texCoord;
void main(){  
    vec2 val = texture2D(u_data, v_index_texCoord).xy*PREC;     // 各通道比例值
    vec2 val1=floor(val * 65535.0 / 256.0); 
    vec2 val2=val*65535.0-val1*256.0;
    gl_FragColor = vec4(val1/255.0,val2/255.0);         // 把两个通道的值转换成四通道
}`;
...
precision highp float;
const float PREC = 255.0/254.0;
uniform sampler2D u_data;       // 生成的存比例值的热力图的纹理==>第一步的帧缓冲纹理
...
void main(){  
        ...
        vec2 point[9];
        point[0]=v_texCoord + vec2(-onePixel.x, onePixel.y);
        point[1]=v_texCoord + vec2(0.0, onePixel.y);
        point[2]=v_texCoord + vec2(onePixel.x, onePixel.y);
        point[3]=v_texCoord + vec2(-onePixel.x, 0.0);
        point[4]=v_texCoord + vec2(0.0, 0.0);
        point[5]=v_texCoord + vec2(onePixel.x, 0.0);
        point[6]=v_texCoord + vec2(-onePixel.x, -onePixel.y);
        point[7]=v_texCoord + vec2(0.0, -onePixel.y);
        point[8]=v_texCoord + vec2(onePixel.x, -onePixel.y);
        float kernelWeight;
        vec2 val;
        bool invalid=false;     // 判断是否在边框，边框就不卷积
        for(int i=0;i<9;i++){
...
const frag1 = `
precision highp float;
uniform sampler2D u_frameImage;     // 第二步的帧缓冲纹理
...
class Meteo{
    ...
    _init() {
        ...
        map.on('resize', (e) => {
            ...
            this.fboKernel = this.initFramebufferObject(this.gl);
    ...
    _initGL() {
        ...
        const vertShaderKernel = createShader(gl, gl.VERTEX_SHADER, vertKernel);
        const fragShaderKernel = createShader(gl, gl.FRAGMENT_SHADER, fragKernel);
        this.programKernel = createProgram(gl, vertShaderKernel, fragShaderKernel);
        ...
        const fboKernel=this.fboKernel = this.initFramebufferObject(this.gl);
        if (!fboKernel) {
            console.log('Failed to intialize the framebuffer object (FBO)');
            return;
        }
    ...
    load(url, vector){
        ...
        MeteoImage.load(url).then((meteo) => {
            ...
            this.gl.uniform1fv(this.programKernel["u_kernel[0]"], new Float32Array([1.0/16.0,2.0/16.0,1.0/16.0,
    ...
    showHLPointsWithMapbox(){
        ...
        for (let v of la.values())
            ps.push(turf.point([v[1], v[2]], {v:-1,val:Math.round(min+(max-min)*v[0]/254),
                description:"L\r\n" +(Math.round(min+(max-min)*v[0]/254)-273.0).toFixed(0)    // fixme:单位百帕
            }));
        for (let v of ha.values())
            ps.push(turf.point([v[1], v[2]], {v:1,val:Math.round(min+(max-min)*v[0]/254),
                description:"H\n\r" +(Math.round(min+(max-min)*v[0]/254)-273.0).toFixed(0)    // fixme:单位百帕
            }));
    ...
    getLonlatFromLonLatImage(index){
        ...
        // if(currentLonLatRate[0]<=0.01||currentLonLatRate[0]>=0.99||currentLonLatRate[1]<=0.01||currentLonLatRate[1]>=0.99){
        /!*if(currentLonLatRate[0]<=0||currentLonLatRate[0]>=1||currentLonLatRate[1]<=0||currentLonLatRate[1]>=1){
            return null;
        }*!/
    ...
    _frame(){
        ...
        // fixme:(2)帧缓冲区卷积
        this.gl.activeTexture(this.gl.TEXTURE0+TEXTURE_FRAMEKERNEL); // Set a texture object to the texture unit
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.fboKernel.texture); // fixme:这里放的是帧缓冲区的纹理图像
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.fboKernel);
        gl.viewport(0, 0, this.gl.canvas.width, this.gl.canvas.height);
        this.gl.clearColor(0, 0, 0, 0);
        this.gl.clear(this.gl.COLOR_BUFFER_BIT);
        gl.useProgram(this.programKernel);
        if(this.fontInfo.hasBorder){
            gl.uniform1i(this.programKernel.u_hasBorder, 1);
        }else{
            gl.uniform1i(this.programKernel.u_hasBorder, 0);
        }
        bindAttribute(gl, this.posBuffer, this.programKernel.a_pos, 2);
        bindAttribute(gl, this.texBuffer, this.programKernel.a_texCoord, 2);
        this.gl.uniform2fv(this.programKernel.u_textureSize, [this.gl.canvas.width, this.gl.canvas.height]);
        gl.uniform1i(this.programKernel.u_data, TEXTURE_FRAMEBUFFER);
        ...
        gl.uniform1i(this.program1.u_frameImage, TEXTURE_FRAMEKERNEL);
            ...
            if(fontArray.length===2){
                ...
                if(tp[1]<1.0&&tp[1]>0.0){
                    ...
                    this.geojson1.features.push({
                        "type": "Feature",
                        "properties": {
                            "description": fontArray[1]-273.0,    // 单位是百帕
                        },
                        "geometry": {
                            "type": "Point",
                            "coordinates": [lon,lat]
