...
const vec2 u_num=vec2(5.0,5.0); // 画几乘几区域的文字
const float u_precision=9.0;    // 保留几位小数
const int maxNumLength=7;   // todo：最多显示几位有效数字，这里float是7-8位有效数字，暂时设成只显示最多7位有效数字

const float a=1.0/u_num.x;  // 每组文字的所在区域范围==>坐标宽度
const float b=1.0/u_num.y;  // 每组文字的所在区域范围==>坐标高度
const float c1=a*0.3;       // 每组文字的显示区域范围==>坐标宽度
const float c2=b*0.1;       // 每组文字的显示区域范围==>坐标高度
 
/*const int numLength=int(log2(abs(-98765.23))/log2(10.0)+1.0);  // 有几个整数位// log10(x)+1
const int preci=numLength+int(u_precision);   // 总共几位有效数字
const float fontNum=u_precision>0.0?float(preci+2):float(preci+1);  // 画几个字体，正负符号+有效数字+小数点
const float d=c1/fontNum;   // 每个字体占显示区域范围的范围==>坐标宽度*/

vec4 getFontColor(vec2 texCoord,float value,float digit,float d){
    if (value >= ((digit>=10.0)?(digit):(0.0))   ) {
        ...
        texCoord.x = getNumber(value, digit) / 16.0 + xOffset;
...
void main(){
    ...
    if(tp.y<1.0&&tp.y>0.0){
        ...
        if(valid(c)){ 
            ...
            const float test=-0.000123456789;
            const float value = test;   // 要显示的数值
            const int numLength=int(((log2(abs(value))/log2(10.0)>=0.0)?(log2(abs(value))/log2(10.0)):(0.0))+1.0);  // 有几个整数位==>必须大于等于0 // log10(x)+1
            const int preci=numLength+int(u_precision);   // 总共几位有效数字
            /*float fontNum=2.0;    // fixme:fontNum用于for循环，所以要在前面加const限定字
            if(preci<=maxNumLength){    // fixme：总的需要有效位数<=最大能显示的有效位数
                fontNum=(u_precision<=0.0)?float(preci+1):float(preci+2);  // 画几个字体，正负符号+有效数字+小数点
            }else if(preci>maxNumLength){   // fixme：总的需要有效位数>最大能显示的有效位数
                if(numLength>=maxNumLength){    // fixme:整数位>=7位，就按整数位来，小数位不显示了
                    fontNum=float(numLength+1);
                }else{
                    fontNum=float(maxNumLength+2);
                }
            }*/
            const float fontNum=(preci<=maxNumLength)?((u_precision<=0.0)?float(preci+1):float(preci+2)):((numLength>=maxNumLength)?(float(numLength+1)):(float(maxNumLength+2)));  // 画几个字体，正负符号+有效数字+小数点
            const float d=c1/fontNum;   // 每个字体占显示区域范围的范围==>坐标宽度
            vec4 fontColor;
            if(mod(texCoord.x,a)<=c1&&mod(texCoord.y,b)<=c2){
                for(int i=0;i<int(fontNum);i++){    // fixme:for循环中的变量一定是const常量表达式，否则编译不通过
                    if(mod(texCoord.x,a)/c1<d/c1*float(i+1)){
                        if(i+1==1){   // 符号位
                            if(value<0.0){   // 负数
                                float xOffset = mod(mod(texCoord.x,a),d)/d*(1.0/60.0);       
                                float yOffset = mod(texCoord.y,b)/c2; 
                                texCoord.x = 11.0 / 16.0 + xOffset;
                                texCoord.y = 1.0-(0.0 + yOffset);
                                fontColor = vec4(1.0,1.0,1.0,2.0)-texture2D(u_numbers, texCoord);
                            }else{  // 非负数
                                fontColor = vec4(1.0,1.0,1.0,1.0);
                            }
                        }else if(i+1>=2&&i+1<=numLength+1){ // 整数位
                            if(i>=maxNumLength+1){  // 大于float的7位+符号位，就显示0
                                float xOffset = mod(mod(texCoord.x,a),d)/d*(1.0/16.0);      
                                float yOffset = mod(texCoord.y,b)/c2; 
                                texCoord.x = 0.0 / 16.0 + xOffset;
                                texCoord.y = 1.0-(0.0 + yOffset);
                                fontColor = vec4(1.0,1.0,1.0,2.0)-texture2D(u_numbers, texCoord);
                            }else{  // 正常显示
                                fontColor =getFontColor(texCoord,abs(value),pow(10.0,float(numLength-i)),d);
                            }
                        }else if(i+1==numLength+2){ // 小数点
                            float xOffset = mod(mod(texCoord.x,a),d)/d*(1.0/17.0);       
                            float yOffset = mod(texCoord.y,b)/c2; 
                            texCoord.x = 10.0 / 16.0 + xOffset;
                            texCoord.y = 1.0-(0.0 + yOffset);
                            fontColor = vec4(1.0,1.0,1.0,2.0)-texture2D(u_numbers, texCoord);
                        }else if(i+1>=numLength+3){ // 小数位
                            fontColor =getFontColor(texCoord,abs(value),pow(10.0,float(numLength+1-i)),d);
                        }
                        break;
                    }
                }
