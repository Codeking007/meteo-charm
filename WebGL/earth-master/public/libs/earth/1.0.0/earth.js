...
(function () {
    ...
    function buildMesh(resource) {
        console.error("buildMesh(resource)==> resource:"+resource);
    ...
    function buildGlobe(projectionName) {
        console.error("buildGlobe(projectionName)==> projectionName:"+projectionName);
    ...
    function buildGrids() {
        ...
        return when.all(loaded).then(function (products) {  // products==>FACTORIES配置
            log.time("build grids");
    ...
    function createMask(globe) {
        ...
        context.fillStyle = "rgb(11,255,199)";
        context.fill();
    ...
    function interpolateField(globe, grids) {
        ...
        function interpolateColumn(x) {     // 全屏屏幕像素点在第几列
            var column = [];
            let count=0;
            for (var y = bounds.y; y <= bounds.yMax; y += 1) {
                if (mask.isVisible(x, y)) {
                    point[0] = x;
                    point[1] = y;
                    var coord = projection.invert(point);   // 像素点转换成经纬度
                    var color = TRANSPARENT_BLACK;
                    var wind = null;
                    if (coord) {
                        var λ = coord[0], φ = coord[1];
                        if (isFinite(λ)) {
                            wind = interpolate(λ, φ);       // 根据经纬度得到相应经纬度点的气象数据
                            var scalar = null;
                            if (wind) {
                     ...
                     column[y] = wind || HOLE_VECTOR;    // 这时的风uv是带显示效果移动比例的，如果没气象数据就返回[NaN, NaN, null]
                    mask.set(x, y, color);
                }
            }
             columns[x] = column;
    ...
    function animate(globe, field, grids) {
        ...
        function evolve() {
            ...
            particles.forEach(function (particle) {
                if (particle.age > MAX_PARTICLE_AGE) {  // 如果年龄超100了就重新生成粒子
                ...
                if (m === null) {   // 如果没有值就是已经不再球体内了，就让它直接消失重新生成粒子
                    ...
                    var xt = x + v[0];
                    var yt = y + v[1];
                    if (field.isDefined(xt, yt)) {  // 如果要到的那个像素点也是有uv值，就加到buckets里
                        // Path from (x,y) to (xt,yt) is visible, so add this particle to the appropriate draw bucket.
                        particle.xt = xt;
                        particle.yt = yt;
                        buckets[colorStyles.indexFor(m)].push(particle);
                    } else {                        // 如果要到的那个像素点没有uv值，即不在可见范围内时，就让它变为起点，等下次再进来时，就走if (m === null)的方法了，就消失重新生成粒子了
        ...
        g.lineWidth = PARTICLE_LINE_WIDTH;  // 动画的线宽
    ...
    function init() {
        ...
        d3.select("#scale").attr("width", (d3.select("#menu").node().offsetWidth - label.offsetWidth) * 0.97).attr("height", label.offsetHeight / 2);

        d3.select("#show-menu").on("click", function () {
            if (!µ.isEmbeddedInIFrame()) {  // 如果没被嵌在IFrame中，就显示/隐藏菜单栏
