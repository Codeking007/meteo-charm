...
var products = function() {
    ...
    function bilinearInterpolateVector(x, y, g00, g10, g01, g11) {  // Δx,Δy,g00, g10, g01, g11
        var rx = (1 - x);
        var ry = (1 - y);
    ...
    function buildGrid(builder) {
        ...
        var Δλ = header.dx, Δφ = header.dy;    // distance between grid points (e.g., 1 deg lon, 1 deg lat)
        var ni = header.nx, nj = header.ny;    // number of grid points W-E and N-S (e.g., 360 x 181)
        ...
        for (var j = 0; j < nj; j++) {  // 数据索引行
            var row = [];
            for (var i = 0; i < ni; i++, p++) { // 数据索引列
        ...
        function interpolate(λ, φ) {    // 根据经纬度得到相应经纬度点的气象数据
            var i = µ.floorMod(λ - λ0, 360) / Δλ;  // 数据索引坐标i // calculate longitude index in wrapped range [0, 360)
            var j = (φ0 - φ) / Δφ;                 // 数据索引坐标j // calculate latitude index in direction +90 to -90
